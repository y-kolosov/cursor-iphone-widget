...
- сумма on-demand, только если перерасход включён и `used > 0`
...
- заголовок `Cursor`
- бар Cursor Models + целое `%`
- бар Other Models + целое `%`
- дата сброса (`d MMM`, локаль устройства)
- on-demand на small **нет** (не влезает)
...
- On-demand виден на medium только при ненулевом перерасходе.
